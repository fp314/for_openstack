...
#遍历继承树将，进行fields属性融合；设置_obj_+属性的get/set/del方法
def _make_class_properties(cls):
    ...
    cls.fields = dict(cls.fields)   #确保是dict ?
    for supercls in cls.mro()[1:-1]:    #遍历继承树将 fields属性融合
        if not hasattr(supercls, 'fields'):
            continue
        for name, field in supercls.fields.items():
            if name not in cls.fields:
                cls.fields[name] = field
    for name, field in cls.fields.items():  #检查fields是否合格
        if not isinstance(field, obj_fields.Field):
            raise exception.ObjectFieldInvalid(
                field=name, objname=cls.obj_name())

        def getter(self, name=name):
            attrname = _get_attrname(name)  #_obj_ + name
        ...
        def setter(self, value, name=name, field=field):
            ...
            field_value = field.coerce(self, name, value)   #格式化数据
            if field.read_only and hasattr(self, attrname): #只读不能修改
            ...
            self._changed_fields.add(name)  #记录改变
            try:
...
class VersionedObjectRegistry(object):
    ...
    def __new__(cls, *args, **kwargs):    #确保创建实例_registry及_registry._obj_classes
        if not VersionedObjectRegistry._registry:
            ...
            VersionedObjectRegistry._registry._obj_classes = \
                collections.defaultdict(list)    #字典类型，但value为list
    ...
    def registration_hook(self, cls, index):
        pass
    #将cls注册放入到_registry._obj_classes字典中
    def _register_class(self, cls):
        def _vers_tuple(obj):
            return vutils.convert_version_to_tuple(obj.VERSION)

        _make_class_properties(cls)     #遍历继承树将，进行fields属性融合；设置_obj_+属性的get/set/del方法
        obj_name = cls.obj_name()       #cls.__name__
        for i, obj in enumerate(self._obj_classes[obj_name]):
            self.registration_hook(cls, i)
            if cls.VERSION == obj.VERSION:      #版本一样者覆盖
                self._obj_classes[obj_name][i] = cls
                break
            if _vers_tuple(cls) > _vers_tuple(obj):     #版本大者插入，版本大的在前面
                # Insert before.
...
#基类主要用于实现obj--->dic(primitive)和primitive-->obj,     remoted  and RPC  类似序列化；反序列化
class VersionedObject(object):
    ...
    #          }
    fields = {}     #用于现在，属性的类型， 不存数据  ， dic{属性名称：类型}
    obj_extra_fields = []
    ...
    def __init__(self, context=None, **kwargs):
        self._changed_fields = set()        #属相改变集合set
    ...
    @classmethod #为field加前缀versioned_object.field
    def _obj_primitive_key(cls, field):
        return '%s.%s' % (cls.OBJ_SERIAL_NAMESPACE, field)

    @classmethod #primitive提供数据，获取field对应的值
    ...
    @classmethod #在注册类VersionedObjectRegistry中获取特定版本objver的类objname
    ...
    @classmethod #primitive提供数据，构建一个VersionedObject实例
    ...
    @classmethod #以上两个方法的结合，1、从primitive中获取namespace、类名（name）、版本（version）；2、根据类名、版本从VersionedObjectRegistry获取类；3、结合类+primitive的data及changes生成实例
    def obj_from_primitive(cls, primitive, context=None):
        ...
        if objns != cls.OBJ_PROJECT_NAMESPACE:  #OBJ_PROJECT_NAMESPACE = 'versionedobjects'
    ...
    def __deepcopy__(self, memo):   #深度copy
    ...
    def _obj_relationship_for(self, field, target_version):
        ...
        if (not hasattr(self, '_obj_version_manifest') or   #_obj_version_manifest属性与obj_relationships功能类似负责多版本映射
...
#本类VersionedObject的primitive数据结构：字典｛versioned_object.name:~;versioned_object.namespace:versionedobjects;versioned_object.version:~;versioned_object.data:primite;versioned_object.changes:~｝
#其versioned_object.data为核心数据，其结构为字典｛fields.key:value｝,如果fields.key为Object者继续嵌套;如果版本不一致进行兼容处理。
    def obj_to_primitive(self, target_version=None, version_manifest=None):
        ...
        return obj
    #为default != obj_fields.UnspecifiedDefault的设置默认值
    ...
    #嵌套查看变化的field，通过_changed_fields ;   keys
    ...
    #返回_obj_name是否为self的属性  ？？？？？？？？？？    _obj_name???????????????
...
#返回离tgt_version最近的版本（趋向大的），并backport_func处理，
